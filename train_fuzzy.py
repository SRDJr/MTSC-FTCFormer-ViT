...
def main():
    ...
    if Config.USE_SOFTMAX:
        print(f"   Pipeline: FANTF-Style (Softmax Activated)")
    else:
        print(f"   Pipeline: Softmax-Free (Phase 2)")
    print(f"==================================================\n")
    ...
    # ==========================================
    # --- THESIS PROOF: EXTRACT FUZZY VALUES ---
    # ==========================================
    if Config.USE_FUZZY:
        print("\n" + "="*50)
        print("   EXTRACTING OPTIMIZED FUZZY PARAMETERS")
        print("="*50)
        
        # 1. Print the Learned Rules (Mu and Sigma)
        for name, param in model.named_parameters():
            if 'mu' in name or 'sigma' in name:
                # We detach and convert to list for clean printing
                vals = param.data.squeeze().cpu().numpy().round(3).tolist()
                print(f"[{name}] Learned Values: {vals}")
                
        # 2. Extract Final Membership Tensors (From the deepest layer)
        # We grab a single batch from the test_loader to run through the network
        model.eval()
        with torch.no_grad():
            images, _ = next(iter(test_loader))
            images = images.to(Config.DEVICE) - 0.5
            _ = model(images) # Forward pass to populate the weights
            
            # Reach into the deepest TCBlock (block4, last element)
            final_memberships = model.block4[-1].last_fuzzy_weights
            
            print(f"\nExtracted Membership Tensor Shape: {final_memberships.shape}")
            
            # Save the tensor for heatmap generation later
            tensor_path = os.path.join(Config.BASE_DIR, "results", "final_memberships.pt")
            torch.save(final_memberships.cpu(), tensor_path)
            print(f"✅ Successfully saved fuzzy memberships to: {tensor_path}")
